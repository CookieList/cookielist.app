...
class BadgeSnippet(StandaloneTag):
    safe_output = True
    tags = {"snippet"}
    ...
    @staticmethod
    def __highlight_snippet(match: re.Match):
        match_groups: dict[str, str] = match.groupdict()
        match_class = list(
            filter(lambda group: match_groups[group], match_groups.keys())
        )[0]
        if match_class == "TEXT":
            match_groups[match_class] = (
                match_groups[match_class].replace(">", "&gt;").replace("<", "&lt;")
            )
        return f'<span class="{match_class}-snippet">{match_groups[match_class]}</span>'

    def render(self, snippet: str, **kwargs):
        return self._SNIPPET_REGEX.sub(
            self.__highlight_snippet, snippet.format(**kwargs)
        )
