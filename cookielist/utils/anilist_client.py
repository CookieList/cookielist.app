...
class AnilistClient:
    ...
    def query(
        ...
        if (
            ...
            try:
                error_message = content.get("errors", dict(message=response.text))
                error_message = error_message[-1]["message"] if isinstance(error_message, list) else error_message["message"]
            except:
                error_message = response.text
                
            try:
                error_status = content.get("errors", dict(status=response.status_code))
                error_status = error_status[-1]["status"] if isinstance(error_status, list) else error_status["status"]
            except:
            ...
            return self.query(operation=operation, authorization=authorization, retry=retry - 1, **variables)
